...
void IRAM_ATTR flagging_the_CLK_state()
{
  static unsigned long int last_time_CLK = 0;
  unsigned long int current_time_CLK = millis();
  if(current_time_CLK - last_time_CLK < 5)
  {
    return;
  }
  is_CLK_triggered = true;
  last_time_CLK = current_time_CLK;
}
void IRAM_ATTR flagging_the_SW_state()
{
  static unsigned long int last_time_SW = 0;
  unsigned long int current_time_SW = millis();
  if(current_time_SW - last_time_SW < 50)
  {
    return;
  }
  is_SW_triggered = true;
  last_time_SW = current_time_SW;
}
